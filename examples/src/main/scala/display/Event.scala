...
/** The mouse moved and/or a button on it was pressed or released. A `Clicked`
  * event is taken to be a pair of `Pressed` `Released` events in sequence,
  * repeated possibly many times. All the events are reported: the programmer
  * should choose between responding to `Clicked` or to `Pressed` and `Released`
  * events.
  */
class MouseEventDetail[D](jdk: event.MouseEvent, pixelsPerUnitLength: Double)
    extends EventDetail[D] {
...
/** A mousewheel-like control has been rotated. Some operating systems (OS/X in
  * particular) report two-fingered "dragging" on a tablet as mouse wheel
  * rotations.
  */
class MouseWheelEventDetail[D](
    jdk: event.MouseWheelEvent,
...
/** A key was typed when the `Display`s window had the focus.
  */
class KeyEventDetail[D](jdk: event.KeyEvent, pixelsPerUnitLength: Double)
    extends EventDetail[D] {
...
case class ComponentHidden[D](detail: ComponentEventDetail[D]) extends Event[D]
case class ComponentShown[D](detail: ComponentEventDetail[D]) extends Event[D]
case class ComponentMoved[D](detail: ComponentEventDetail[D]) extends Event[D]
case class ComponentResized[D](detail: ComponentEventDetail[D]) extends Event[D]

...
/** Immutable cartesian coordinates in 3-space. Only subclasses have
  * write-access to the underlying representation
  *
  * @see
  *   `Vector.Variable`
  */
class Value(
    protected[Vector] var _x: Double = 0.0,
  ...
  @inline
  def squareTo(that: Value): Double =
    sq(that._x - _x) + sq(that._y - _y) + sq(that._z - _z)
...
object Value {
  def apply(x: Double = 0.0, y: Double = 0.0, z: Double = 0.0): Value =
    new Value(x, y, z)
  val Zero = new Value()
}

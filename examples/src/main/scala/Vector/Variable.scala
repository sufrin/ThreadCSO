...
class Variable(vx: Double = 0.0, vy: Double = 0.0, vz: Double = 0.0)
    extends Value(vx, vy, vz) {
  ...
  /** Assign `that` to `this` */
  def :=(that: Value): Unit = { _x = that._x; _y = that._y; _z = that._z }
  def boundAbove(that: Value): Unit = {
    _x = math.min(_x, that._x); _y = math.min(_y, that._y);
    _z = math.min(_z, that._z)
  }
  def boundBelow(that: Value): Unit = {
    _x = math.max(_x, that._x); _y = math.max(_y, that._y);
    _z = math.max(_z, that._z)
  }
...
object Variable {
  def apply(vx: Double = 0.0, vy: Double = 0.0, vz: Double = 0.0): Variable =
    new Variable(vx, vy, vz)
}

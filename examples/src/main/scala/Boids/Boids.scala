 ...
 *
 *  Simplified interaction with `Boid`  characteristics uses the keyboard.
 *  This could be refined to incorporate a GUI, but life's too short.
 *
 */
object Boids {
  ...
  { var N = 250
    ...
    def setParam(arg: String): Unit = {
        ...
        if (arg == "S+") BoidDisplay.Synchronizing(true) else
        if (arg == "S-") BoidDisplay.Synchronizing(false) else
        if (arg == "A-") BoidDisplay.antiAlias=false else
        if (arg == "A+") BoidDisplay.antiAlias=true else
        ...
        if (arg.startsWith("-d")) debugPort = arg.substring(2).toInt else
        Console.println(f"""
          S${if (BoidDisplay.Synchronizing) "+" else "-"}\t\tsynchronising (+) or not (-)
          A${if (BoidDisplay.antiAlias) "+" else "-"}\t\tantialiasing (+) or not (-)

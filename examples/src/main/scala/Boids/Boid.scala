...
class Boid {
  ...
  @inline private def mag(x: Double, y: Double): Double = sqrt(
    square(x) + square(y)
  )
  ...
  @inline private def norm(x1: Double, size: Double): Double =
    if (abs(x1) > abs(x1 + size)) x1 + size
    else if (abs(x1) > abs(x1 - size)) x1 - size
    else x1
  ...
  private def isInView(x1: Double, y1: Double): Boolean = {
    ...
    val offSet =
      min(offSet0, 2 * Pi - offSet0); // angle between direction and direction1
  ...
  def newState(allBoids: Iterable[Boid]): Boid.State = {
    var forceX = 0.0;
    var forceY = 0.0 // cohesion and separation force components
    ...
    for (b <- allBoids; if b != this) {
      if (isInView(b.x, b.y)) {
        ...
        forceX += (coh - sep / distSquared) * dx;
        forceY += (coh - sep / distSquared) * dy
    ...
    val newTheta =
      if (count > 0) Alignp * avTheta + (1.0 - Alignp) * direction0
      else direction0

...
object Autonomous extends App {
  ...
  /**
    ...
    */
  def worker(me: Int, self: Body, others: Iterable[Body]): PROC =
    proc(s"worker($me)") {
      // initialize
      ...
      while (true) { // update local state
        ...
        for (other <- others if other ne self) {
          val force =
            (self attractionTo other) * (if (self touches other) ballBounce else 1.0)
          localForce += force
        }
        barrier.sync()
    ...
    repeat {
      val e = fromDisplay ? {
        ...
        case KeyPressed(key) =>
          ...
          if (changed) display.draw(syncWait=false)
        case _ => {}
      }
    }
  }
  ...
  def Main(): Unit = {
    ...
    barrier = new Barrier(allBodies.length)

    val workers: PROC = ||(for { wid <- 0 until allBodies.length} yield worker(wid, allBodies(wid), allBodies))

...
/** This little, almost pointless, example was intended to let us explore the
  * practicality of working with autonomous interacting bodies that have
  * different characteristics, when their interactions are governed by physical
  * as well as logical laws.
  *
  ...
  * workers that run in synchrony, this example associates each body with its
  * own "internal" controller.
  *
  * Implementing gravitation-like forces can turn out to be rather inefficient
  * if on every "tick" of the display it is necessary to compute all N*N
  * inter-body forces. The cost can be halved by noting that the force of "A" on
  * "B" is the opposite of the force of "B" on "A". We don't do that here.
  *
  * Autonomous bodies
  */
object Autonomous extends App {
  ...
  object GUI {
    ...
    val display = new Display[Body](
      ...
      North = controls,
      South = row(
        reports withTitledBorder "Reports",
        hGlue,
        overruns withTitledBorder "Overruns"
      )
    )
  }
  ...
  def Main(): Unit = {
    ...
    if (allBodies.isEmpty) {
      allBodies += new Body(
        60,
        new Position(width * 0.5, height * 0.66),
        new Velocity(-1, +1, 0)
      )
      allBodies += new Body(
        90,
        new Position(width * 0.5, height * 0.33),
        new Velocity(-1, +1, 0)
      )
      allBodies += new Body(
        30,
        new Position(width * 0.75, height * 0.25),
        new Velocity(-1, +1, 0)
      )
      allBodies += new Body(
        20,
        new Position(width * 0.75, height * 0.75),
        new Velocity(+1, +1, 0)
      )
    }

    val bodies: PROC = ||(for {
      wid <- 0 until allBodies.length
    } yield allBodies(wid).controller)

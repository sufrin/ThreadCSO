...
/**
        It is noticeable how infrequently it turns out to be necessary
        for the first fork to be put down because the second fork
        isn't forthcoming. The backoff vs. eating-time ratio is
        (probably) a significant factor in this. Backing off if the
        first fork isn't available improved the retry frequency
        aspect of the performance of the simulation below, though
        I haven't measured ''throughput''.
*/
...
object PhilsProto 
{
  ...
  def Phil(me: Int, left: ForkChan, right: ForkChan) = proc("Phil"+me) {
    ...
    repeat {
      ...
      report!(s"$me dropping left fork $lf"); left!Drop(me);  report!(s"$me dropped left fork $lf"); Pause
      report!(s"$me dropping right fork $rf"); right!Drop(me); report!(s"$me drops right fork $rf"); Pause
    } 
    println(s"Phil $me DIED")
  }
  
  object Owned extends Enumeration
  { val LEFT, RIGHT, NOBODY = Value }
      ...
      {(  left.fromClient  =?=> 
          { case Pick(x) => 
            ...
            if (owned==Owned.LEFT && y==owner) {
               ...
               throw new Error(s"fork $me (owned by $owner/$owned) receives Drop($y)")
          }
       |  right.fromClient =?=> 
          { case Pick(x) => 
            if (owned==Owned.NOBODY) {
  ...
  val philToLeftFork  = 
      for (i<-0 until N) yield 
          forkChan(s"Phil($i) to Fork($i)")
  val philToRightFork = 
      for (i<-0 until N) yield 
          forkChan(s"Phil($i) to Fork(${(N+i-1)%N})")

...
/** <p>
  * Elementary trials of concurrent composition, thread-pooling, thread stacksize specification.
  *
  * Also Primes:
  {{{
...
object PAR extends APITrial {
  def MAIN(args: Array[String]): Unit =
  { val p = ||(for (arg<-args) yield
                  proc(arg) {
                     Console.println(arg)
                     sleep(1000)
                     if (arg==".") stop
                     else
                     if (arg.startsWith(".")) throw new Throwable(arg)
                  }
                )
    Console.println(p)
    p()
...
/** Primes generator: tests propagation of forward and backward close, and of kernel thread-creation limits
        ...
        || proc("output")
           { var i=1
             ...
             { System.out.println("%04d %5d".format(i, res?()))
               i=i+1
               if (i==count) stop
               if (pause!=0 && i>pause) { Console.print("<Enter> to continue: "); readLine; () }
             }
             Console.println("FINISHED")
             res.closeOut()
           }.withStackSize(aSize)
        || proc("source")
           { var i=2; repeat { mid!i; i=i+1; if (i==max) mid.closeIn() } }.withStackSize(aSize)
        )()
     }
     finally {
        exit()
  ...
  def sieve(name: String)(in: channel.?[Int], out: channel.![Int]) : PROC =
      proc (name)
      {
        ...
        { out!n
          ...
          if (showClose) Console.println("(nomult(%d}||sieve ...)() finished".format(n))
          in.closeIn()
          mid.close()
          out.closeOut()
        }
...
object Chan1 extends APITrial {
  def MAIN(args: Array[String]): Unit =
  { val mid = if (args contains ("-a")) OneOne[String]("mid") else io.threadcso.channel.OneOne[String]("mid")
...
/** <p>Tests downstream transmission and closing,
  * upstream closing; and extended rendezvous.
  *
  * Sender repeatedly prompts, reads a keyboard line, sends it to mid
  * Listener reads from mid and (in an extended rendezvous)
  * prints the line (without NL) pauses and prints a newline.
  *
...
/**
        <p> Various tests of OneOne or  `N2N(writers, readers=1)` used as an intermediate
        channel between (possibly many) writers/senders and a listener process.

        The constant factor is that the listener process uses extended rendezvous, and thus
        keeps the {{{send-N}}} processes engaged for 500ms after they send.
        This is shorter than the time taken to start a concurrently-running
        {{{send-N}}}, and this means that a OneOne gets '''output-barged'''. Of course
        a {{{ManyOne}}} doesn't, for any contention to output to the channel is
        resolved in the synchronized wait for !.
{{{
        //
...
object Chan6 extends APITrial {
  ...
  { val mid : Chan[String] =
    ...
    val sendParClose = || (for (arg<-args) yield π ("send-"+arg) {mid!arg; mid.closeOut()})
    val sendSeq = π { for (arg<-args) π ("send-"+arg) {mid!arg} ()  }
    val send = if (args contains "-s") sendSeq else
               if (args contains "-n") sendParClose  else sendPar
    run (  π { send()
               Console.println("---")
...
object Chan7 extends APITrial {
  ...
  { def showWait(s: String): String =
    ...
    val mid : Chan[String] = if (args contains "-m")
                        N2N[String](readers=1, writers=0, name="mid")
                     else
                        OneOne[String]("mid")
    val recPar = (||(for (arg<-args) yield
                         π ("rec-"+arg) {showWait(mid?())}))
    val recSeq = π { for (arg<-args) run(π ("rec-"+arg) { showWait(mid?()) }) }
    run (  π { for (arg<-args) mid!arg
               Console.println("---")
               mid.closeOut()
             }
...
object Deadlock extends APITrial {
 def fwd(in: ?[String], out: ![String]): PROC = proc (s"forward ${in.name} to ${out.name}")
 {
    repeat { val v = in?(); Console.println(in.name); out!v }
 }
 def MAIN(args: Array[String]): Unit =
 { val chans = for (arg<-args) yield OneOne[String](arg)
   val procs = || (for (i<-chans.indices)
...
/** <p> Test transitivity of extended rendezvous.
  * Writer sends a * into a pipeline of forwarding processes (of length the number of arguments).
  * Reader reads from the end of the pipeline, then awaits input from the console.
  * Writer shows it has synchronized by printing #.
  * then the pipeline of forwarders closes down.
  *
  */
object Rendezvous extends APITrial {
 ...
 { val left  = OneOne[String]("LEFT")
   ...
   val chans = List(left) ++ args.map(arg=>OneOne[String](arg)) ++ List(right)
   val procs = || (for (i<-0 until chans.length-1)
                       yield fwd(s"${chans(i).name} -> ${chans(i+1).name}")(chans(i), chans(i+1)))
   run(  procs
      || π("Writer"){ left!"*"; Console.println("#"); left.closeOut() }
      || π("Reader"){ right??{s => Console.println(s"$s"); readLine}; println("Reader finished")  }
      )
   exit()
 }
}
...
/**     <p>
        Basic buffer implementation test for use with debugger.

        -s    args -- sequential output of args into buffer
                      (>10 will deadlock)
                      followed by sequential input from buffer
        args       -- concurrent output and input of args; terminates at end of args.
        -step args -- as above; but prompt for <enter> at each output.
        Receiver does a closeInp on receiving a .
        When not -stepping sender is likely to get more data into the buffer
        than when stepping.
*/

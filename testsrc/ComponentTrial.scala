...
/**
  <p> A mixin to deal with the parsing of arguments specifying parameters of one or more component trials.
{{{
...
abstract class ComponentTrial(doc: String)(implicit loc: SourceLocation) extends App
{ implicit class MkString[T](s: Seq[T]) {
  ...
  ,   OPT("-m",     messages,                s"«int» number of messages to invent and send ($messages) in Messages")
  ,   OPT("-s",     senders,                 s"«n» number of senders ($senders)")
  ,   OPT("-r",     receivers,               s"«n» number of receivers ($receivers)")
  ,   OPT("-b",     bufSize,                 s"«n» radius of buffers ($bufSize) [-1 means use synchronized channel; 0 means unbounded]")
  ,   OPT("-N",     N,                       s"«n» set N ($N)")
  ...
  def tr(event: Any)(implicit location: SourceLocation): Unit = log.log(1, event.toString)(location) 
  ...
  { if (debug) {
    ...
    if (poolwait>0)  java.lang.System.setProperty("io.threadcso.pool.SECS", poolwait.toString)
    val beginTime = nanoTime
    MAIN
    println(s"$Command ($loc) terminated: elapsed ${(nanoTime-beginTime).hms}")
    if (debug) println("Inspect traces using the debugger") else exit()
  }

  def Pipe[T](name: String): Chan[T] = if (bufSize<0) OneOne[T](name) else OneOneBuf(bufSize, name)
}
...
object TimeStamped extends ComponentTrial("TimeStamped -- echos time-stamped keyboard lines to console")
{ def MAIN
  { var n    = 0
    val kbd  = OneOne[String]("kbd")
    ...
    run(       keyboard(kbd, { n+=1; s"$n> "})
            || zip(tick, kbd, mid)
            || map[(Nanoseconds, String), String]({case (n: Nanoseconds, s: String) => s"$s @ ${n.hms}"})(mid, out)
            || console(out)
       )
  }
}
...
object Ticked extends ComponentTrial("Ticked -- echos time-stamped ticks to console")
{ def MAIN
  { var n    = 0
    ...
    val out    = OneOne[String]("out")
    if (debug) debugger.monitor("n",{ n.toString })
    run( zip(   timer, ticker, mid)
             || map[(Nanoseconds, Unit), String]({case (n: Nanoseconds, _ : Unit) => s"@ ${n.hms}"})(mid, out)
             || console(out)
       )
  }
}

/** A light test of shared channels. Runs writers in parallel with readers transmitting args  down an N2N/N2NBuf mid*/
...
/** A test of shared channels. Runs writers in parallel with readers: transmitting messages down an N2N/N2NBuf mid, without printing them.
  * Active readers contend for messages, active writers contend for the channel.
  * */
object Messages extends ComponentTrial("Messages")
{ val n = new java.util.concurrent.atomic.AtomicInteger(senders)
  import io.SourcePath._

  /** write `messages+1` successive integers to `out` (log once every 1000) then `out.closeOut` */
  def writer(me: Int, out: channel.![Int]): PROC =
      π(s"writer($me)")
      {
        repeatFor(0 until messages)
        {
          arg =>
            out ! arg
  ...
  /** While there are readers active read messages from `in`, and log if a message is a multiple of 1000. Log
    * (and diminish the count of active readers) if the message is the number `messages`. */
...
/** Copies named files to the console -- interleaving lines ad-hoc if there is more than one */
object FileCopy extends ComponentTrial("FileCopy")
{
  def MAIN: Unit =
  {
    val term: Chan[String] = N2N[String](writers=args.length, readers=1, name="terminal")
...
/** Runs `N` target loops repeatedly printing their identities. Responds to a typed identity by interrupting it */
...
/**
  This is a tad slower than commscso
  because the channels are referenced as free variables.
 ...
 {{{
        ...
        The channels (--> and <-- in the diagram) can be
        synchronized or asynchronous. When asynchronous
        the radius of the buffers can be set.
  ...
  def MAIN {
    ...
    val Sink =
      proc("Sink") {
        ...
        println(s"Cold $caption: $trials trials of $N tokens (injecting $inject) took ${elapsed * 1.0}ns @ ${(elapsed * 1.0) / (1.0E3 * N * trials)}μs cycle")

        elapsed = 0
        for (_ <- 0 until trials) trial()
        println(s"Warm $caption: $trials trials of $N tokens (injecting $inject) took ${elapsed * 1.0}ns @ ${(elapsed * 1.0) / (1.0E3 * N * trials)}μs cycle")

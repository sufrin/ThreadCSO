...
/** A general-purpose shared buffered channel, bounded by `size` if it is
  ...
  * simultaneously, and (unlike [[OneOne]] channel) there is no dynamic check
  * for reader-overtaking (more than a single process trying to read
  * simultaneously) or writer overtaking.
  *
  * It closes completely after `closeIn` has been invoked, or after `closeOut`

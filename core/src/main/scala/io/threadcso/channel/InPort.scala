...
/** A channel is a sequential conduit for data that is written to its `OutPort`,
  ...
  * conditions of congestion: precise behaviours of such transport should be
  * documented with their definitions.
  */
trait InPort[+T] {
  ...
  /** Block until a value ''t'' is available for input, then return
    * `f(`''t''`)`. If the inport is provided by a synchronized channel then
    * synchronisation with the termination of the sender's `!` is at the
    * termination of the computation of `f(`''t''`)` (this is sometimes called
    * an ''extended rendezvous'').
    */
  def ??[U](f: T => U): U

  /** Signal that no further attempts will be made to input from the invoking
    * thread: ''idempotent''
    */
  def closeIn(): Unit

  /** Returns `false` if no further input will be supplied by this port.
    * Returning `true` is not a guarantee that further input will be supplied.
    */
  def canInput: Boolean

  /** The null value for this port -- equivalent to `null.asInstanceOf[T]`.
    * Useful to initialise variables that will in due course be assigned values
    * by reading from the port.
    */

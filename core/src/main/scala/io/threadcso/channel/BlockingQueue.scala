...
/** A blocking queue used in the implementation of buffers. If `bound==0` then
  * the queue is unbounded and `put` will never be blocked.
  *
  * @param bound
  *   (if positive) bound on the size of the queue: a put will stall if the
  *   queue reaches this size.
  * @param parent_
  *   the component whose implementation uses this queue (for the debugger)
  * @tparam T
  *   the type of value in the queue
  */

private[channel] class BlockingQueue[T](
    var bound: Int,
    parent_ : AnyRef,
    name: String
) {
  private[this] val homeGrown =
    true // Jan 9th 2018 ==> false works for MagicSquares
  ...
  private[this] val enqMutex =
    BooleanSemaphore(true, null, fair = false, parent_, spin = 3)
  private[this] val deqMutex =
    BooleanSemaphore(true, null, fair = false, parent_, spin = 3)
  ...
  @inline def size: Int =
    length // { enqMutex.acquire; try { length } finally { enqMutex.release } }
  ...
  /** Put the given `value` in the queue; stalling if necessary until space is
    * available
    */
  ...
  /** If space is available before `nsWait` elapses, put `value` in the queue,
    * and return true; otherwise return false. This always returns true for an
    * unbounded buffer.
    */
  ...
  /** If a datum (`v` say) is available at the front of the queue before
    * `nsWait` elapses, then dequeue it and return `Some(v)`; otherwise return
    * `None`
    */
  ...
  /** Unconditionally and atomically remove and return the first element of the
    * list
    */
  ...
  /** Remove and return the longest-waiting value in the queue; stalling if

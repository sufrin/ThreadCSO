...
/** A `PROC` is a prototype for a process that can be started and run. In
  * ''Threaded CSO'' a `PROC` is started by acquiring a thread from an executor:
  * this may be orthodox thread pool or (in later implementations) source of
  * virtual threads.
  *
  * The executor may be specified by `withExecutor` as the `PROC` is
  * constructed, or (as is much more usual) may be provided by default just as
  * the `PROC` starts running.
  */
trait PROC extends (() => Unit) {
  ...
  def withExecutor(anExecutor: CSOExecutor): PROC = {
    executor = anExecutor; this
  }

  def withExecutor(anExecutorName: String): PROC =
    withExecutor(CSOThreads.getExecutor(anExecutorName))

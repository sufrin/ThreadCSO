...
object CSOThreads {
  ...
  /** PRE: size of CSO platform thread pools (or count of CSO threads) is not
    * increasing EFFECT: applies f to each active thread in the CSO pool.
    */
  ...
  /** PRE: size of CSO virtual thread pool is not increasing EFFECT: applies f
    * to each active virtual thread.
    */
  ...
  /** Shut all executors down. TODO: This could be somewhat more efficient.
    * (Only shut down the used executors) It's only used to debrief the pooled
    * executors.
    */
  def shutDown(): Unit = {

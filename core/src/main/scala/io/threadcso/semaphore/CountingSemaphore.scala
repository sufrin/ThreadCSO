...
/** A counting semaphore implemented with a nonblocking queue. In principle the
  * semaphore manages a finite collection of (identical) tokens.
  *
  * The object `parent` (by default the semaphore itself) is used by the
  * debugger when reporting the state of waiting processes.
  *
  * The value of `available` may be negative, in which case it represents a
  * deficit of tokens.
  *
  * The value `spin` is the number of attempts the semaphore's local spinlock
  * will make to acquire a token before a call to `acquire` or `acquireFast`
...
class CountingSemaphore(
  ...
  override def toString: String =
    s"""$name: ${_count.get} available [${_waiting.length} ${_waiting.elements
        .map(_.identity)
        .mkString(", ")}]"""

  /** Interrupt each thread waiting to acquire this semaphore, and cause it to
    * throw an `InterruptedException`. Mark the semaphore itself cancelled
    */
  override def cancel() = {
    _cancelled = true
  ...
  /** Return true iff a token has been acquired with a short busy wait. This
    * avoids entering the scheduler (which is costly) when a rival that owns the
    * semaphore is not going to stay long.
    */
  ...
  /** Wait until a token is (or becomes) available. If the acquiring thread is
    * cancelled during the wait or the semaphore's `cancel()` method has been
    * called, then rethrow the cancel.
    */
  def acquire(): Unit = {
  ...
  @inline private[this] def atomicDec: Boolean = _count.getAndUpdate({ n =>
    if (n > 0) n - 1 else n
  }) > 0
  ...
  /** Add a token to the collection, then permit an `acquire`ing process to
    ...
    */
  def release(): Unit = { if (_count.incrementAndGet > 0) signal }
  ...
  /** Wait until a token becomes available (returning true) or until the
    ...
    * mark the semaphore as having been cancelled
    */
...
object CountingSemaphore extends NameGenerator("CountingSemaphore") {
  ...
  /** Construct a boolean semaphore.
    ...
    *   [[io.threadcso.semaphore.jdk.CountingSemaphore]]
    * @param parent
  ...
  def apply(
    ...
    if (fair)
      new io.threadcso.semaphore.jdk.CountingSemaphore(
        available,
        name,
        true,
        parent != null

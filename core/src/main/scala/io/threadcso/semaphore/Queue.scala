...
private[semaphore] trait Queue[T] {
  ...
  /** Remove and return the first element in the queue (or null if the queue is
    * empty)
    */
  def dequeue(): T
  ...
  /** Approximation to the current sequence of elements in the queue (for
    * debugging)
    */

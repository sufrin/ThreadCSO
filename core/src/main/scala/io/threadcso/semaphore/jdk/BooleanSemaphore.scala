...
/** A classical boolean semaphore that manages a single virtual ''permit''. Each
  * `acquire` waits if necessary until the permit is available, and then makes
  * it unavailable (its "owner" is said to be the process that last made it
  * unavailable). A `release` makes the permit available (if it wasn't already)
  * to a waiting process.
  *
  * If the semaphore is `fair` the semaphore will (under contention) guarantee
  * first-in first-out acquisition of the permit. Fairness can impose very
  * considerable delays.
  *
  * A thread that `release`s the permit ''need not have acquired it
  * previously''.
  *
  * This implementation delegates most of its implementation to
...
class BooleanSemaphore(
    ...
    _register: Boolean = false
      ...
      BooleanSemaphore.genName(_name),
      fair,
      _register
    ) {
  private[this] val owner =
    new AtomicReference[Thread](if (available) null else Thread.currentThread())
  override def toString = s"""$name ${if (owner.get == null) "available"
    else s"owner: ${owner.get.identity}"} [waiting: ${getWaiting
      .map(_.identity)
      .mkString(", ")}]"""

  override def acquire(): Unit = {
    super.acquire(); owner.set(Thread.currentThread())
  }
  override def release(): Unit =
    if (owner.getAndSet(null) != null) super.release()

  override def tryAcquire(wait: Long): Boolean = if (super.tryAcquire(wait)) {
    owner.set(Thread.currentThread()); true
  } else false

  //////// debugger interface

  override def showState(out: java.io.PrintWriter): Unit =
    out.print(s"BSEMA $this")

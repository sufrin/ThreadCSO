...
/** A high-performance variant of `Flag` with no intrinsic support for debugging
  * or dynamic error-checking. A synchronization object that supports a
  * ''single'' thread waiting until some other thread enables it to proceed.
...
class FastFlag extends Semaphore {
  ...
  override def toString: String =
    s"""FASTFLAG: ${if (_available.get) "available" else "unavailable"} ${if (
        _interrupted
      ) "[cancelled]"

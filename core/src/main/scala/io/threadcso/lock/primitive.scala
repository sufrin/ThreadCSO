...
/** Primitive high-performance synchronised channels with no debugger interface
  * or closing protocol; and no protection from inadvertent sharing.
  *
  * These are mainly intended to be used in the implementation of
  * Barriers/CombiningBarriers -- to avoid the need for ad-hoc communication
  * mediated by ad-hoc networks of semaphores.
  *
  * In what is perhaps a fit of self-indulgence, we provide the usual "standard"
  * channel read and write operations.
  */

object primitive {

  /** A `primitive.Chan[T]` may be in one of the two conceptual states: EMPTY,
    * or FULL(t:T)
    */
  trait Chan[T] {
  ...
  /** An initially-EMPTY primitive.Chan[Unit]. Equivalent to a DataChan[Unit],

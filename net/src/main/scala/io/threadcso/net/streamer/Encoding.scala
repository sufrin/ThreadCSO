...
import io.threadcso.net.streamer.VarInt._
  ...
  /**
    ...
    */
    ...
    implicit object `VarInt*` extends Streamer[VarInt] {
      def toStream(out: DataOutputStream, t: VarInt): Unit  = writeVarInt(out, t)
      def fromStream(in: DataInputStream): VarInt = readVarInt(in)
      val name = "`VarInt*`"
    }
